...
score_before = function(new_game, runs_on_play, top_inning, home_team = 1) {
  m = length(new_game)
  home_score_before = numeric(m)
  away_score_before = numeric(m)
  
  for (i in 2:m) {
    if (new_game[i] == 0) {
      if (top_inning[i-1] == 0) {
        home_score_before[i] = home_score_before[i-1] + runs_on_play[i-1]
        away_score_before[i] = away_score_before[i-1]
      } else {
        away_score_before[i] = away_score_before[i-1] + runs_on_play[i-1]
